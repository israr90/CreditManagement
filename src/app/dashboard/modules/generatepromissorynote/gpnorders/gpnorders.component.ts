...
@Component({
  ...
  templateUrl: './GPnorders.component.html',
  styleUrls: ['./gpnorders.component.scss'],
})
export class GPnOrdersComponent implements OnInit {
  ...
  TabsCollection = [
    { TabNo: 1, Name: ' Generate Promissory Notes' },
    { TabNo: 2, Name: 'Reschedule Promissory Notes ' },
    { TabNo: 3, Name: 'Transfer Promissory Notes ' }
  ];
  constructor(
 ...
 debugger;
    this.router.navigateByUrl('/gpnorders/generate/'+orderId);
  }
  ...
  chooseBranch(item: any, index: number) {
    this.select = item.TabNo;
    debugger
    if(this.select==1){
      this.PNOrderType='18001';
    }
    else if(this.select==2){
      this.PNOrderType='18002';
    }
   else if(this.select==3){
      this.PNOrderType='18003';
    }   
     this.GetPNOrdersList()
