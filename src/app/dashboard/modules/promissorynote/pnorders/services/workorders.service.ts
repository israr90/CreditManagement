...
export class WorkOdersService {
  ...
  GeneratePromissoryNotes(id: number,Guarantor:boolean,GuarantorName:string,GuarantorPhone:string,GuarantorIDNo:string,IDFile:File,SecretFile:File,Notes:string) {
    return this.http.get(
      this.apiUrl +
        `PNOrders/GeneratePromissoryNotes?OrderId=${id}&Guarantor =${Guarantor}&GuarantorName=${GuarantorName}&GuarantorName=${GuarantorName}&GuarantorPhone=${GuarantorPhone}&GuarantorIDNo=${GuarantorIDNo}&IDFile=${IDFile}&SecretFile=${SecretFile}&Notes=${Notes}`,
      this.sharedService.getHeaders()
    );
  }
